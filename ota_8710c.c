...
int update_ota_erase_upg_region(uint32_t img_len, uint32_t NewFWLen, uint32_t NewFWAddr){
	...
	if(NewFWLen == 0){
		...
		if((int)NewFWLen > 0){
            ...
            {
                printf("Erase block %d addr 0x%08X\r\n", i, NewFWAddr + i * 4096);
				flash_erase_sector(&flash_ota, NewFWAddr + i * 4096);
                vTaskDelay(10); // for internal wdt
            }
...
int update_ota_http_connect_server(int server_socket, char *host, int port){
    ...
    if(server == NULL){ 
      printf("[ERROR] %s : %d :  Get host %s ip failed\n", __FILE__, __LINE__, host);
		return -1;
    }
		...
		for (i = 0; i < response_len; ++i) {
			...
			if (response[i] == '\r' && response[i+1] == '\n' &&
				...
				if(result->parse_status == 2){//get the full header and the content length
					...
					result->header_len = header_end;
					result->body = response + header_end;
                    printf("Get full header and content len, remain%d\r\n", response_len-i);
				}
...
int parse_http_response_clone(uint8_t *response, uint32_t response_len, http_response_result_t *result, uint8_t **remain_data, uint32_t * remain_size) {
    uint32_t i, p, q, m;
    uint32_t header_end = 0;
...
//                    printf("Get full header and content len, remain %d\r\n", response_len-i);
                    ...
                    *remain_data = response + i + 4;
                    printf("Remain size %d, next 4 byte 0x%02X 0x%02X 0x%02X 0x%02X\r\n", *remain_size, response[i+4], response[i+5], response[i+6], response[i+7]);
				}
				else {//there are no content length in header	
					printf("\n\r[%s] No Content-Length in header", __FUNCTION__);
					return -1;
...
int http_update_ota(char *host, int port, char *resource)
{
	...
	if(NewFWAddr != ~0x0){
		...
		//send http request
		request = (unsigned char *) update_malloc(1024);
//		sprintf((char*)request, "GET /%s HTTP/1.1\r\nHost: %s\r\n\r\n", resource, host);
//        sprintf((char*)request, "GET /%s HTTP/1.1\r\nHost: %s\r\nAuthorization: %s\r\n\r\n", resource, host, "Basic YWRtaW46YWRtaW4=");       // huyTV
        // huytv
        char * cp = request;
        if(cp == NULL) { printf("NULL pointer\r\n"); while(1) vTaskDelay(1000); }
		...
		while (idx < NewFWLen){
			...
			while(read_bytes < data_len){
                ...
                {
                        if(first_4_byte == 0 && read_bytes)
                        {
                          first_4_byte = 1;
                          printf("First 4 byte 0x%08X 0x%08X 0x%08X 0x%08X", buf[0], buf[1], buf[2], buf[3]);
                        }
//                     printf("read_bytes %d\r\n", read_bytes);
                }
			}
...
int http_update_ota_clone(char *host, int port, char *resource)
{
    // to do : add checksum
	int server_socket = -1;
	unsigned char *buf = NULL, *request = NULL;
	...
	if(NewFWAddr != ~0x0){
        ...
        {//still read header
			if(0 == rsp_result.parse_status){//didn't get the http response
				...
				memset(&rsp_result, 0, sizeof(rsp_result));
                printf("11111\r\n");
				if(parse_http_response_clone(buf, idx, &rsp_result, &remain_data, &remain_byte_after_parse_header) == -1){
					goto update_ota_exit;
				}
			}
			else if((1 == rsp_result.parse_status) || (3 == rsp_result.parse_status))
            {//just get the status code
                ...
                printf("2222\r\n");
				if(parse_http_response_clone(buf, read_bytes + HEADER_BAK_LEN, &rsp_result, &remain_data, &remain_byte_after_parse_header) == -1){
					goto update_ota_exit;
				}
			}
		}
        ...
        if(remain_byte_after_parse_header)
        {
            ...
            {
                printf("0x%02X ", remain_data[i]);
            }
            printf("\r\n");
          
            device_mutex_lock(RT_DEV_LOCK_FLASH);
			if(flash_burst_write(&flash_ota, address, remain_byte_after_parse_header, remain_data) < 0){
				printf("\n\r[%s] Write sector failed", __FUNCTION__);
				device_mutex_unlock(RT_DEV_LOCK_FLASH);
				goto update_ota_exit;
			}
        ...
        vTaskDelay(10);
        idx = remain_byte_after_parse_header;
        memset(buf, 0, BUF_SIZE);
		while (idx < NewFWLen){
			printf("Recv %d\r\n", idx);
			data_len = NewFWLen - idx;
            ...
            bool expire = 0;
			while(read_bytes < data_len && expire == 0){
				read_bytes += read(server_socket, &buf[read_bytes], data_len-read_bytes);
				if(read_bytes == 0){
					printf("\n\r[%s] Read socket failed", __FUNCTION__);
					goto update_ota_exit;
                ...
                {
                    if(current_tick > (5000 + previous_tick) )
                    {
                        expire = 1;
                    }
                }
                else if(previous_tick > current_tick)
                {
                   if((portMAX_DELAY - previous_tick + current_tick) > 5000)
                   {
                        expire = 1;
                   }
                }
        ...
        {
            printf("Update signature success\r\n");
        }
	}
update_ota_exit:
